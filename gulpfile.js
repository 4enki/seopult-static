...
/*---------- Tasks ----------*/
  ...
  // Шаблонизация для раздела
  gulp.task('html:gui', function() {
      return gulp.src(paths.source.templates + 'page/guides/*.html')
          .pipe(plumber(plugins.plumber))
          .pipe(include())
          .pipe(gulp.dest(paths.build.html + 'guides'));
  });
  ...
  // Компиляция стилей
  gulp.task('styles:gui', function () {
      return gulp.src(paths.source.styles + 'guides/guides.sss')
          .pipe(plumber(plugins.plumber))
          .pipe(changed(paths.build.styles))
          .pipe(postcss(processors, { parser: sugarss }))
          .pipe(rucksack())
          .pipe(rename('guides.css'))
          .pipe(gulp.dest(paths.build.styles + '/guides'))
          .pipe(_if(argv.prod, cssnano()))
          .pipe(_if(argv.prod, rename('guides.min.css')))
          .pipe(_if(argv.prod, gulp.dest(paths.build.styles + '/guides')));
  });
...
/*---------- Бдительные вотчеры ----------*/
  ...
  gulp.task('watch', function() {
    ...
    watch(paths.source.templates + 'page/guides/**/*.html', function() {
        return runSequence('html:gui', browserSync.reload);
    });
    watch(paths.watch.styles, function() {
      return runSequence('styles', browserSync.reload);
    });
    watch(paths.source.styles + 'guides/guides.sss', function() {
        return runSequence('styles:gui', browserSync.reload);
    });
...
/*---------- Режимы ----------*/
  ...
  gulp.task('default', function(cb) {
    return runSequence(
      'copy',
      ['html', 'html:gui', 'styles', 'styles:gui', 'scripts', 'watch'],
      'server',
      cb
    );
  });
